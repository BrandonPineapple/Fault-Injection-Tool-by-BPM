...
class FaultInjector:
    ...
    # ---------- implementación genérica para RAM/REGISTRO (orden corregido) ----------
    def _inj_memoria(self, falla, tamano_bytes, delay):
        """
        Versión corregida: coloca los breakpoints ANTES del resume.
        Evita que el core ejecute instrucciones antes de activar los comparadores FPB.
        """
        if self.core is None:
        ...
        # ---------- RESET Y ARMADO DE BREAKPOINTS EN ORDEN CORRECTO ----------
        try:
            self.core.reset_and_halt()
            print("[INFO] MCU reseteado y detenido (RAM/registro)")
        except Exception as e:
            print(f"[WARNING] No se pudo hacer reset_and_halt(): {e}")
        ...
        # 3) colocar BP temporal de esta falla
        try:
            self.core.set_breakpoint(bp_addr, self.core.BreakpointType.HW)
            print(f"[INFO] BP temporal armado: 0x{bp_addr:08X}")
        except Exception as e:
        ...
        # 4) mostrar breakpoints activos
        try:
            activos = [hex(getattr(b, 'address', 0)) for b in self.core.get_breakpoints()]
            print(f"[DEBUG] Breakpoints activos: {activos}")
        except Exception:
            pass

        # 5) reanudar ejecución (ahora sí)
        try:
            self.core.resume()
            print("[INFO] MCU reanudado, esperando alcanzar BP temporal…")
        except Exception as e:
            print(f"[WARNING] No se pudo resume(): {e}")

        # ---------- BUCLE DE ESPERA DEL BP TEMPORAL ----------
        start = time.time()
        while True:
            if time.time() - start > 5.0:
                ...
                try:
                    ...
                    pc = None
                print(f"[⚠️] TIMEOUT esperando BP temporal (PC=0x{(pc or 0):08X})")
                if pc != self.stop_address:
                    try:
                        self.log_falla(falla, None, None, None, "HANG_NO_LLEGO_A_BP")
                    except Exception:
                        pass
                    self.hangs += 1
                    try:
                        self.core.reset_and_halt()
                        print("[INFO] MCU reiniciado tras HANG")
                    except Exception:
                        pass
                return
            ...
            if self.core.is_halted():
                ...
                if pc == bp_addr:
                    ...
                    if self.stop_address:
                        try:
                            self.core.set_breakpoint(self.stop_address, self.core.BreakpointType.HW)
                            print(f"[INFO] stop_address rearmado: 0x{self.stop_address:08X}")
                        except Exception as e:
                            print(f"[WARNING] No se pudo rearmar stop_address: {e}")

                    try:
                        self.core.resume()
                        print("[INFO] Reanudando ejecución tras inyección…")
                    except Exception:
                        pass
