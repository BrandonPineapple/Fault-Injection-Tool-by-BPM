...
from M_detector_campaña import obtener_ultima_carpeta_campania
...
def grafico_clasificacion_tecnica(df, output_path):
    ...
    plt.title("Clasificación técnica (GOLD vs AFTER_STABLE)", fontsize=16)
    plt.tight_layout()

    out = os.path.join(output_path, "grafico_clasificacion_tecnica.png")
    plt.savefig(out)
    plt.close()
...
def grafico_inyector_estados(faultlog, output_path):
    ...
    def map_categoria(e):
        ...
        if "NO_ESCRITA" in e:
            return "No escrita"
        if "NO_APLICADA" in e:
            return "No aplicada, no llegó al BP de inyeccion"
        if "NO_LEIDO" in e:
            return "No leído"
        return "Otro"
    ...
    plt.title("Estados del Inyector (por Fault_ID)", fontsize=16)
    plt.tight_layout()

    out = os.path.join(output_path, "grafico_inyector_estados.png")
    plt.savefig(out)
    plt.close()
