...
class VentanaCSV(QDialog):
    ...
    def _seleccionar_archivo(self):
        ...
        try:
            # Leer con UTF-8-SIG para eliminar BOM de Windows
            with open(archivo, "r", encoding="utf-8-sig") as f:
                ...
                if reader.fieldnames is None:
                    QMessageBox.warning(
                        ...
                        "No se pudo leer el encabezado del archivo CSV."
                    )
                    return

                # Normalizar encabezados (quita BOM y espacios)
                ...
                if header != COLUMNAS_ESPERADAS:
                    QMessageBox.warning(
                        self, "Formato incorrecto",
                        f"El CSV no coincide con el formato esperado.\n\n"
                        f"Detectado: {header}\n"
                        f"Esperado: {COLUMNAS_ESPERADAS}"
                    )
                    return

                # Si el CSV es válido, llenar lista
